...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title[The $\beta$-skeleton and the T-web]{Cosmic web classification of galaxies using the $\beta$-skeleton}
...
\begin{abstract}
We show how it is possible to infer the environments of the cosmic web of dark matter using the information of the spatial distribution of galaxies and a machine learning algorithm. Handled the set of galaxies as a set of nodes, we applied the $\beta$-skeleton algorithm to extract geometrical information like the number of connections or the average distance by galaxy. Using this properties of the distribution of galaxies was probed the classification trees algorithm under different configurations to find the best set of parameters that makes a good prediction with the lowest confusion. The results show that the local average distance between galaxies on the $\beta$-skeleton is the most important feature for the prediction of the cosmic web environment. This is a good approximation that will allow us to use these algorithms to make predictions in observational data, like SDSS and DESI.
\end{abstract}
...
\section{Initial Algorithms and Simulations}
...
\section{Linking the $\beta$-skeleton to the T-Web}
...
scale of $\approx1.2$ \Mpch and an threshold $\lambda_{th}=0.1$. 
...
magnitude cut, althout a cut of $M_r>-14$ gives the best results.

The two best algorithm to predict the environment are the classification
...
The environments that are best predicted are in decreasing order: filaments, peaks, sheets and
voids. 
This ranking follows the number of galaxies found in each of these environments.
Galaxies in voids represent less that $2\%$ of the total number of galaxies and are the
hardest class to predict, with $F1$-scores of $\approx0.2$.
Motivated by this limitation, in future work we will present a method to define voids 
from the $\beta$-skeleton graph.

All the results we reported here were obtained using a single snapshot at a redshift $z=0$.
Our results provide the baseline for future work that will have to quantify the effect of
redshift space distortions and survey incompleteness in order to predict 
the DM T-Web form survey data.
Nevertheless, the most appealing feature of our method is that its does not rely either on binning the galaxy data on a grid or computing a dark matter density reconstruction.
It only requires the computation of the $1$-skeleton graph and the application of
simple cuts on the features we have reported.
